...
function toDisplayString(v) {
  ...
  if (typeof v === 'object') {
    return Object.entries(v).slice(0, 3).map(([k, val]) => `${k}: ${val}`).join(', ');
  }
  return String(v);
}
...
function ProbableCausesChart({ causes }) {
  ...
  return (
    <div>
      <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-2.5">Top Probable Causes</p>
        ...
        {causes.map((c, i) => {
          ...
          return (
            <div key={i} className={`px-3 py-2.5 rounded-lg border ${
              isTop ? 'bg-orange-950/25 border-orange-800/50' : 'bg-slate-700/10 border-slate-700/40'
            }`}>
...
function EvidenceBasis({ reasoning, items }) {
  ...
  return (
    <div className="space-y-2">
      {reasoning && <p className="text-slate-400 text-xs leading-relaxed">{reasoning}</p>}
      {hasItems && (
          ...
          <tbody className="divide-y divide-slate-700/40">
            {rows.map((r, i) => (
              <tr key={i} className={r.supports_rca ? 'bg-green-950/20' : 'hover:bg-slate-700/10'}>
                ...
                <td className="px-3 py-2 text-slate-300 max-w-[200px] truncate" title={r.declared_cause}>{r.declared_cause}</td>
                <td className="px-3 py-2">
                  <span className={`px-1.5 py-0.5 rounded text-xs ${
                    r.category === 'unknown' ? 'bg-slate-700 text-slate-400' : 'bg-indigo-900/40 text-indigo-300 border border-indigo-800/40'
                  }`}>{r.category}</span>
                </td>
                <td className="px-3 py-2"><SeverityBadge severity={r.severity} /></td>
                <td className="px-3 py-2 text-center">
                  {r.supports_rca ? <span className="text-green-400 font-bold">✓</span> : <span className="text-slate-600">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
...
function AgentWorkflow({ steps }) {
  ...
  return (
    <Section title="Multi-Agent Execution" icon={<FiActivity className="text-indigo-400 w-4 h-4" />}
      badge={`${steps.length} agents · ${total.toFixed(1)} ms`} accent="border-indigo-800/40">
              ...
              <div className="flex-1 min-w-0 pb-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-green-400 flex items-center gap-1">
                    <FiCheckCircle className="w-3 h-3" />{step.duration_ms?.toFixed(1)} ms
                  </span>
                  {total > 0 && (
...
function RetrievalHeader({ methodology, isFallback, processingMs }) {
  ...
  return (
    <div className={`rounded-xl border px-4 py-3 ${
      isFallback ? 'bg-slate-800/60 border-slate-600/60' : 'bg-blue-950/40 border-blue-700/50'
    }`}>
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <span className="text-slate-500 text-xs ml-2 hidden sm:inline">
              {isFallback ? `· ${methodology.algorithm}` : '· GPT vector similarity + LLM reasoning'}
            </span>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {(methodology.keywords_used || []).map((kw, i) => (
                <span key={i} className="px-2 py-0.5 bg-indigo-900/30 border border-indigo-800/40 text-indigo-300 rounded">{kw}</span>
              ))}
            </div>
          </div>
          <div>
...
function A2AMessageCard({ msg }) {
  ...
  const displayPayload = Object.entries(msg.payload || {})
    .filter(([k]) => k !== 'raw_results' && k !== 'incidents').slice(0, 5);
  ...
  return (
    <div className="flex gap-3 group">
      ...
      <div className="flex-1 min-w-0 pb-3">
        <button onClick={() => setOpen(o => !o)} className="w-full text-left group/card">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className={`px-1.5 py-0.5 rounded border text-xs font-bold tabular-nums ${style.bg}`}>{style.label}</span>
            <span className="text-xs font-semibold text-slate-300">{from}</span>
            <FiArrowRight className="w-3 h-3 text-slate-600 flex-shrink-0" />
            <span className={`text-xs font-semibold ${msg.to_agent === 'broadcast' ? 'text-amber-400' : 'text-slate-300'}`}>{to}</span>
            {reason && <span className="text-slate-500 text-xs italic truncate max-w-[160px]">{reason}</span>}
            <span className="text-slate-700 text-xs tabular-nums ml-auto">{ts}</span>
          </div>
...
function A2ATimeline({ messages, stats }) {
  ...
  return (
    <Section title="Agent-to-Agent Log" icon={<FiMessageSquare className="text-cyan-400 w-4 h-4" />}
      badge={`${total} messages`} accent="border-cyan-800/40" defaultOpen={false}>
      ...
      <div className="relative pl-1">
        ...
        <div className="space-y-0">
          {messages.map((msg, i) => <A2AMessageCard key={msg.message_id || i} msg={msg} />)}
        </div>
      </div>
      <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-700/40 text-xs text-slate-600">
        {Object.entries(MSG_STYLES).map(([type, s]) => (
          <span key={type} className="flex items-center gap-1.5">
            <span className={`w-2 h-2 rounded-full ${s.dot}`} />{type}
          </span>
        ))}
      </div>
    </Section>
...
export default function ResultsPanel({ results, onIncidentSelect, query, filters }) {
  const [activeResultTab, setActiveResultTab] = useState('summary');
  const [llmLoading, setLlmLoading]           = useState(false);
  const [llmReasoning, setLlmReasoning]       = useState(null);
  const [llmError, setLlmError]               = useState(null);

  const handleEnhanceWithLlama = async () => {
    setLlmLoading(true);
    setLlmError(null);
    try {
      ...
      const reasoning = res.data?.root_cause_analysis?.llm_reasoning;
      setLlmReasoning(reasoning || 'No reasoning returned.');
    } catch (err) {
      const raw = err.response?.data?.detail || err.message || 'AI request failed';
      const msg = raw.includes('timeout') || raw.includes('timed out')
        ? 'AI gateway timed out — check internet or API key'
        : raw;
      setLlmError(msg);
    } finally {
      setLlmLoading(false);
  ...
  return (
    <div className="space-y-3 animate-fade-in">
      ...
      <RetrievalHeader methodology={methodology} isFallback={isFallback} processingMs={results?.processing_time_ms} />
        ...
        {[
          {
            ...
            accent: 'border-blue-500', valueClass: 'text-white',
          },
          {
            label: 'RCA Confidence', value: `${Math.round(confidence * 100)}%`,
          ...
          {
            label: 'Priority Level', value: priority.toUpperCase(),
          ...
          {
            label: 'Est. Resolution', value: resolution.estimated_resolution_time?.split('–')[0] || '—',
            sub: resolution.estimated_resolution_time
              ? `up to ${resolution.estimated_resolution_time.split('–')[1] || ''}`
              : 'based on history',
            accent: 'border-green-500', valueClass: 'text-white',
          },
        ].map(({ label, value, sub, accent, valueClass }) => (
          <div key={label} className={`bg-slate-800/80 rounded-xl border-l-4 border border-slate-700 ${accent} p-3 shadow-sm`}>
      ...
      {/* Llama enhance panel */}
      {!llmReasoning ? (
        <div className="rounded-xl border border-slate-700 bg-slate-800/60 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2.5">
            <FiCpu className="w-4 h-4 flex-shrink-0 text-slate-500" />
            <div>
              <span className="text-sm font-semibold text-white">AI Reasoning (LLaMA 3.2)</span>
              <span className="text-slate-500 text-xs ml-2">· LLaMA 3.2 via Ollama (local)</span>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            {llmError && !llmLoading && (
              <span className="text-xs text-amber-400">{llmError}</span>
            )}
            {llmLoading ? (
              <div className="flex items-center gap-2 text-purple-300 text-xs">
                ...
                Reasoning… (~25s on CPU)
              </div>
            ) : (
              <button
                onClick={handleEnhanceWithLlama}
                className="flex items-center gap-2 px-3 py-1.5 bg-purple-700 hover:bg-purple-600 text-white text-xs font-semibold rounded-lg transition-colors"
              >
        ...
        <div className="rounded-xl border border-purple-700/50 bg-purple-950/30 px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-semibold text-white flex items-center gap-2">
              <FiCpu className="w-4 h-4 text-purple-400" />AI Reasoning (LLaMA 3.2)
            </span>
            <span className="text-xs text-purple-400 flex items-center gap-1">
              <FiCheckCircle className="w-3.5 h-3.5" /> Enhanced
            </span>
          </div>
          <p className="text-slate-200 text-sm leading-relaxed">{llmReasoning}</p>
        </div>
      )}

      {/* Sub-tab navigation */}
      <ResultSubTabs active={activeResultTab} onChange={setActiveResultTab} hasWorkflow={hasWorkflow} hasA2A={hasA2A} />

      {/* ── Tab: Root Cause Summary ─────────────────────────────────────── */}
      {activeResultTab === 'summary' && (
        <div className="space-y-3">
          {evidenceSrc === 'query_keywords' && (
            <div className="p-2.5 bg-amber-950/40 border border-amber-700/50 rounded-lg flex items-start gap-2 text-xs">
              ...
              <p className="text-amber-300">
                ...
                RCA inferred from query keywords — confidence is reduced.
              </p>
            </div>
          )}
          ...
          <div className="p-4 bg-orange-950/30 border border-orange-800/40 rounded-xl">
            <p className="text-xs text-orange-400/80 font-semibold uppercase tracking-wide mb-2">Primary Cause</p>
            ...
            {/* LLM reasoning inline */}
            {rootCause.llm_reasoning && (
              <div className="mt-3 pt-3 border-t border-orange-900/40">
                <p className="text-xs text-orange-400/60 font-semibold uppercase tracking-wide mb-1">Llama Reasoning</p>
                <p className="text-slate-300 text-xs leading-relaxed">{rootCause.llm_reasoning}</p>
              </div>
            )}
          </div>
          ...
          {/* Confidence */}
          <div className="bg-slate-800/80 rounded-xl border border-slate-700 p-4 space-y-2">
          ...
          {(rootCause.contributing_factors || []).length > 0 && (
            <div className="bg-slate-800/80 rounded-xl border border-slate-700 p-4">
              <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-3">Contributing Factors</p>
          ...
          {/* Confidence breakdown (collapsed by default) */}
          {confFactors.length > 0 && (
            <details className="group bg-slate-800/80 rounded-xl border border-slate-700 overflow-hidden">
              <summary className="px-5 py-3.5 text-xs text-slate-400 cursor-pointer hover:bg-slate-700/30 flex items-center gap-1.5 select-none list-none">
                <FiInfo className="w-3.5 h-3.5" />Confidence factor breakdown
                <FiChevronDown className="w-3 h-3 group-open:rotate-180 transition-transform ml-auto" />
              </summary>
              <div className="border-t border-slate-700/60 overflow-hidden">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-700/40 text-slate-400 uppercase tracking-wide">
                  ...
                  <tbody className="divide-y divide-slate-700/40">
                    {confFactors.map((f, i) => (
                      <tr key={i} className="hover:bg-slate-700/20">
                        ...
                        <td className="px-3 py-2 text-right text-indigo-400 font-semibold tabular-nums">+{(f.contrib * 100).toFixed(1)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* ── Tab: Impact & Resolution ────────────────────────────────────── */}
      {activeResultTab === 'impact' && (
        <div className="space-y-3">
          <Section title="Service Impact" icon={<FiShield className="text-red-400 w-4 h-4" />}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              ...
              {(impact.affected_vendors || []).length > 0 && (
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1.5">Vendors</p>
                  <div className="flex flex-col gap-1">
                    {impact.affected_vendors.map((v, i) => (
                      <span key={i} className={`px-2 py-0.5 rounded border text-xs ${
                        i === 0 ? 'bg-amber-900/20 border-amber-800/40 text-amber-300' : 'bg-slate-700/30 border-slate-600/40 text-slate-400'
                      }`}>
                        {i === 0 ? '★ ' : ''}{v}
          ...
          <Section title="Resolution Steps" icon={<FiCheckCircle className="text-green-400 w-4 h-4" />}>
            ...
            {resolution.estimated_resolution_time && (
              <div className="flex items-center gap-2 pt-2 border-t border-slate-700/60">
                ...
                {resolution.data_driven && <span className="text-slate-500 text-xs ml-auto">from history</span>}
              </div>
            )}
          </Section>
        </div>
      ...
      {activeResultTab === 'evidence' && (
        <div className="space-y-3">
          <Section title="Retrieved Incidents" badge={`${incidents.length} matches`}
            ...
            {incidents.length === 0 ? (
              ...
              <div className="space-y-2">
                {incidents.map((item, idx) => {
                  ...
                  return (
                    <div key={idx} onClick={() => onIncidentSelect?.(item)}
                      ...
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="font-mono text-xs text-blue-400 font-semibold flex-shrink-0">{inc.alarm_id || `INC-${idx + 1}`}</span>
                      ...
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500">
                        <span className="flex items-center gap-1"><FiMapPin className="w-3 h-3" />{inc.network_region || inc.region}</span>
                        <span className="flex items-center gap-1"><FiWifi className="w-3 h-3" />{inc.technology_type || inc.technology}</span>
                        <span>{inc.device_vendor || inc.vendor}</span>
                        {(inc.outage_duration > 0) && (
                          <span className="ml-auto flex items-center gap-1"><FiClock className="w-3 h-3" />{inc.outage_duration} min</span>
                        )}
                      </div>
                    </div>
                  );
          ...
          {/* Historical table */}
          <div className="bg-slate-800/80 rounded-xl border border-slate-700 px-5 py-4">
          ...
          {(correlation.correlated_alarms || []).length > 0 && (
            <Section
              ...
              <div className="space-y-1.5">
                <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold">Correlation Strength</p>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {correlation.correlated_alarms.map((id, i) => (
                    <span key={i} className="font-mono text-xs px-2 py-1 bg-slate-700/60 text-slate-300 rounded border border-slate-600/60">{id}</span>
                  ))}
                </div>
              </div>
            </Section>
        ...
        <div className="space-y-3">
          ...
          {!hasWorkflow && !hasA2A && (
            <div className="text-center py-12 text-slate-500 text-sm">
              No agent workflow data available for this analysis.
            </div>
          )}
        </div>
      )}
      ...
      {/* Footer */}
      <div className="text-center text-slate-600 text-xs pt-1 pb-2">
        Analysis in <span className="text-slate-500">{(results?.processing_time_ms || 0).toFixed(0)} ms</span>
        {dataSource.matched != null && (
          <> · <span className="text-slate-500">{dataSource.matched} matched</span> of {dataSource.total_incidents}</>
        )}
        {isFallback && <> · BM25 local</>}
      </div>
    </div>
  );
}
